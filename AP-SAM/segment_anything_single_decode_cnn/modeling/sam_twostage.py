...
class Sam_Twostage(nn.Module):
    ...
    def forward(
        ...
        # feature_fusion = self.feature_fusion(interm_embeddings)


        outputs = []
        for image_record, curr_embedding, curr_early_feature,cnn_feature,early_cnn_feature,first_cnn_feature \
                in zip(batched_input, image_embeddings, interm_embeddings,cnn_features,early_cnn_features,first_cnn_features):
            ...
            for i in range(2):
                if i == 0:
                    ...
                    low_res_masks, iou_predictions,upscaled_img_embedding,up_cnn_src, upscaled_pos , ac_token,cnn_token = self.mask_decoder(
