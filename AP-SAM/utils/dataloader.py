...
def get_im_gt_name_dict(datasets, flag='valid'):
    ...
    for i in range(len(datasets)):
        ...
        datalen_list.append(len(tmp_im_list))


    datalen_list.append(counts)
    return name_im_gt_list,datalen_list
...
def create_dataloaders(name_im_gt_list, my_transforms=[], batch_size=1, training=False):
    ...
    if training:
        ...
        #todo decrease the use of gpu memory
...
class ContrastEnhancement(object):
    ...
    def __call__(self, sample):
        ...
        # 对比度增强
        image = torchvision.transforms.functional.adjust_contrast(image/ 255.0, self.factor)
        image = image * 255.0
...
class Tonorm(object):
    def __init__(self,norm_num = 255.0):
        self.norm_num = norm_num
    def __call__(self,sample):

        imidx, image, label, shape =  sample['imidx'], sample['image'], sample['label'], sample['shape']
        image = image / self.norm_num
...
class RandomBrightnessContrast(object):
    def __init__(self, brightness_range=(0.5, 1.5), contrast_range=(0.5, 1.5),prob=0.2):
    ...
    def __call__(self, sample):
        ...
        if random.random() >= self.prob:
            image = image / 255.0
            # random modify brightness
            brightness_factor = random.uniform(*self.brightness_range)
            image = F2.adjust_brightness(image, brightness_factor)

            # random modify contrast
            contrast_factor = random.uniform(*self.contrast_range)
            image = F2.adjust_contrast(image, contrast_factor)
...
class OnlineDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if im.shape[0]!=3:
            print('image_exceptshape',im.shape)
            print('image_path',im_path)
